...
Section "3) inputstatus (WinEventLog section should list Security/System/Application)"
& $splunk list inputstatus -auth "admin:$AdminPw" 2>&1 | Write-Host

Section "4) Which account runs the UF service? (LocalSystem can read the Security log)"
...
Section "5) Recent splunkd.log lines about WinEventLog / event log / TcpOutput"
