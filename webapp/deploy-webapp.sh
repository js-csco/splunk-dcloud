...
# unreachable from other subnets until it transmits again (the "flapping" we
# chased for hours). A 30s ping to the default gateway keeps the MAC/ARP entries
# fresh so the Directory App stays reachable when idle.
...
echo "keepalive started (30s ping -> ${GW:-198.18.3.1}; prevents idle-container flapping)"
