...
The **Customer Example** app (`splunk/apps/customer_example`) turns nine pain points from a
hospital customer's discovery workshop into nine modern **Dashboard Studio** dashboards, plus
an **Overview**. Everything shows a fictional hospital, **Spital Aarwald**, with realistic
sample data: KPI tiles with sparklines, trend and breakdown charts, gauges, and tables with
real-looking hostnames, IPs, wards and tickets. None of the source systems (DHCP, ISE,
Catalyst Center, NSX, bed management, an LLM gateway, …) exist in this lab, so every search
embeds its data with `| makeresults format=csv` (Splunk 9.1+). Timestamps are computed at
search time, so daytime peaks land on daytime and weekend dips on weekends whenever you demo.

- **Overview** — the nine use cases (click through), data sources per use case, and a
  data source × use case matrix: *what we need in Splunk*.
- **Each use case** — the customer's quote, 4 KPIs, 2–4 visuals, a detail table, and a
  strip of the **data sources needed** to build it for real.

| # | Dashboard | Customer's pain point |
|---|---|---|
| 1 | Medical Device IP Drift | An X-ray unit's reserved IP silently flipped to DHCP; long root-cause hunt. |
| 2 | Database Failover Insight | Redundant DB failed over; nobody could say why. |
| 3 | IoT Fleet Health | IoT is ~99% reactive, run with an external partner. |
| 4 | Network Correlation | No correlation across ISE, DNA (Catalyst Center) and switching. |
| 5 | Environmental Device Health | A label printer failed from rising room temperature. |
| 6 | Microsegmentation Change Impact | One click in NSX and "the other side" stops working. |
| 7 | Bed Occupancy | Occupancy view for the executive board (GL). |
| 8 | Swiss Data Residency | "Cloud is fine if processed in Switzerland" — prove it. |
| 9 | AI Observability & Governance | AI adoption on hold due to uncertainty. |

**Edit the generator, not the XML.** The views are generated by
`scripts/build_customer_example.py` (layout, colors, sample data, data-source list):

```bash
python3 scripts/build_customer_example.py   # rewrites splunk/apps/customer_example/.../views/*.xml
```
...
splunk/apps/
  ...
  customer_example/           # 9 Dashboard Studio use-case dashboards + overview (generated by scripts/build_customer_example.py)
...
- [x] Customer Example: 9 Dashboard Studio use-case dashboards + overview for a fictional hospital, realistic sample data, data sources per use case
- [ ] SOAP sender (parked)
```
